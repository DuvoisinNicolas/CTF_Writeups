I made that code using chatGPT mostly (after some iterations).
What it does it build rows for the image.
It takes one random piece as first element, and then compare its rightmost border with others.
If it matches exactly some pixels, it has more weight.
Once this is done, i'll use gimp to make "blocks" of images together.

First i split the image using this script:
```python
from PIL import Image
import os

def split_image_into_grid(image_path, grid_size=(8, 8), output_folder="Puzzle/easy_pieces"):
    """
...
# Example usage
image_path = "Puzzle/puzzle-trouble-easy.jpg"  # Make sure to replace this with the path to your image
split_image_into_grid(image_path)

```

And then, built up images with this one:
```python
from PIL import Image
import os
import math
...
folder_path = "Puzzle/easy_pieces"
...
Here is my output:
![](_attachments/Pasted%20image%2020240405232731.png)

After some gimping:
![](_attachments/Pasted%20image%2020240405234109.png)
